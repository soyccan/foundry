use dotenvy::dotenv;
use etherscan_sync::{proxion::ProxionDatabase, Etherscan, EtherscanSync, SourceCodeDatabase};
use eyre::Result;
use std::env;
...
async fn main() -> Result<()> {
    env_logger::init();
    dotenv()?;
    let etherscan = Etherscan::new(env::var("ETHERSCAN_API_KEY")?.as_str())?;
    let mut source_code_database = SourceCodeDatabase::connect(env::var("DATABASE_URL")?.as_str())?;
    let mut proxion_database =
        ProxionDatabase::connect(env::var("DATABASE_URL_PROXION")?.as_str())?;
    let mut etherscan_sync =
        EtherscanSync::new(&etherscan, &mut source_code_database, &mut proxion_database);
    let addrs = get_address_list_for_test();
    for (address, bytecode_hash) in addrs {
        if let Err(e) = etherscan_sync.sync_source_code_to_database(address, bytecode_hash).await {
            return Err(e);
        }
    }
    Ok(())
...
fn get_address_list_for_test() -> Vec<(&'static str, &'static str)> {
    vec![
        (
